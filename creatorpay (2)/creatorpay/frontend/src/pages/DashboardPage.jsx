...
export default function DashboardPage() {
  ...
  const totalEarned = (user?.balance?.totalEarned || 0) / 100
  const pending = (user?.balance?.pending || 0) / 100
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard label="Total Earned" value={`$${totalEarned.toLocaleString('en-US', { minimumFractionDigits: 2 })}`} change="+18.4%" up color="green" />
        <StatCard label="Available" value={`$${available.toLocaleString('en-US', { minimumFractionDigits: 2 })}`} change="Ready to withdraw" color="purple" />
        <StatCard label="Pending" value={`$${pending.toLocaleString('en-US', { minimumFractionDigits: 2 })}`} change="1–2 days" color="amber" />
        <StatCard label="Active Links" value={user?.stats?.totalLinks || 0} change={`${user?.stats?.totalTransactions || 0} transactions`} color="blue" />
      </div>
